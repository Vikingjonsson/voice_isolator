...
from src.voice_isolator import isolate_voice
...
class TestIsolateVoice(unittest.TestCase):
    ...
    @patch("src.voice_isolator.ElevenLabs")
    @patch("builtins.open", new_callable=mock_open, read_data=b"dummy_audio_data")
    @patch("builtins.print")
    def test_isolate_voice_success_with_bytes(self, mock_print, mock_file, mock_elevenlabs):
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b"dummy_audio_data")
    @patch("builtins.print")
    def test_isolate_voice_success_with_filelike(self, mock_print, mock_file, mock_elevenlabs):
        """Tests successful voice isolation when the API returns a file-like object."""
        mock_client = MagicMock()
        mock_elevenlabs.return_value = mock_client
        mock_client.audio_isolation.convert.return_value = BytesIO(b"isolated_audio_stream")
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b"dummy_audio_data")
    @patch("builtins.print")
    def test_isolate_voice_success_with_iterator(self, mock_print, mock_file, mock_elevenlabs):
        """Tests successful voice isolation when the API returns an iterator."""
        mock_client = MagicMock()
        mock_elevenlabs.return_value = mock_client
        mock_client.audio_isolation.convert.return_value = iter([b"iso", b"lated_", b"audio"])
    ...
    @patch("builtins.open", new_callable=mock_open, read_data=b"dummy_audio_data")
    @patch("builtins.print")
    def test_isolate_voice_unsupported_return_type(self, mock_print, mock_file, mock_elevenlabs):
        """Tests that None is returned for an unsupported API return type."""
        mock_client = MagicMock()
        mock_elevenlabs.return_value = mock_client
        mock_client.audio_isolation.convert.return_value = 12345  # Not bytes, file-like, or iterator

        result = isolate_voice(self.file_path, self.api_key)

        self.assertIsNone(result)
        mock_print.assert_any_call("Voice isolation failed or returned unsupported type.")
